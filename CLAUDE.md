...
**Current generation: pass-6-service-meter / v3.3.0 on `main`** (coordinator-only hot-swap, 2026-07-17; DNA HELD from pass-6/v3.0.0 blessed 2026-07-02; pass-5/v2.0.0 is the migration source generation).
...
```
main-hc060 → pass-1 → pass-2 → pass-2.5 → pass-3 → pass-4 → pass-5 → pass-6 (v3.0.0) → pass-6-pinned-hosts (v3.1.0) → pass-6-idempotent-writes (v3.2.0) → pass-6-service-meter (v3.3.0, main — coordinator generation, DNA HELD)
```
...
- `pass-6` (branch `dry-refactor`, merged → main as v3.0.0) is a structural
  ...
  installed it.
- Main/v3.3.0: DNA `uhC0ksXs…` (HELD), hApp `b98916f1…` —
  pass-6-service-meter coordinator generation: `upsert_service_meter`
  (cumulative max-merge day buckets), `publish_node_spec` (opt-in singleton,
  REPLACE, dormant app-attestation — accepted-keys list ships empty), header
  convergence on upsert, CI cutover to host tests + build + sweettest. Wire
  contract: `docs/HUMM_TAURI_SERVICE_METER_INTEGRATION.md`.
- v3.2.0: DNA `uhC0ksXs…` (HELD), hApp `bfe357aa…` —
