...
Cloned verbatim 2026-05-16 from upstream `main`.

This file is the Oh My Pi (Pi) harness definition. The
`tools:` field references Pi tool primitives (read, search, find,
bash, lsp, web_search, ast_grep) and `model: pi/smol` selects a Pi
model. If you load this repository under a different harness, you
will need to translate those two fields:

- Claude Code / Cursor / similar: rename tools to that harness's
  equivalents (Read, Grep, Glob, Bash, WebFetch in Claude Code's
  case) and set `model: inherit` or your harness's default.
- The body (system prompt + procedure + directives) ports without
  change.

The harness Mike runs day-to-day (Oh My Pi) provides a built-in
`librarian` agent type and resolves this file automatically. The
file is committed here so future contributors using other
harnesses can still load the same behaviour.
-->
