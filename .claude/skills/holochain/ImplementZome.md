...
> **Tryorama (TypeScript) — deprecated.** Existing Tryorama suites generated by `hc scaffold happ` still run, but Tryorama is not the recommended path for new test work. If you must run a legacy Tryorama suite: `bun run test:foundation` / `bun run test:integration`.
